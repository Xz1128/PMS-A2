 ...
 */
function renderItemsTable(items, containerId, title) {
    ...
    items.forEach((item) => {
        ...
        const stockClass = item.stockStatus === 'In Stock' ? 'status-in-stock' :
            item.stockStatus === 'Low Stock' ? 'status-low-stock' : 'status-out-of-stock';
        // Determine popular badge
        const popularBadge = item.popularItem === 'Yes'
...
function renderEditForm(item) {
    ...
    const categoryOptions = VALID_CATEGORIES.map((cat) => `<option value="${cat}" ${cat === item.category ? 'selected' : ''}>${cat}</option>`).join('');
    const stockOptions = VALID_STOCK_STATUSES.map((status) => `<option value="${status}" ${status === item.stockStatus ? 'selected' : ''}>${status}</option>`).join('');
    const popularOptions = VALID_POPULAR_OPTIONS.map((opt) => `<option value="${opt}" ${opt === item.popularItem ? 'selected' : ''}>${opt}</option>`).join('');
    formContainer.innerHTML = `
...
function getFormData() {
    return {
        ...
        category: getInputValue('input-category'),
        ...
        stockStatus: getInputValue('input-stock'),
        popularItem: getInputValue('input-popular'),
